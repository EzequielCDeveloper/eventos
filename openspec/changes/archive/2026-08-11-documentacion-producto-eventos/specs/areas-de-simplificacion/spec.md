...
Cada proveedor SHALL elegir su modo de aprobación: manual (contrato físico para salones) o inmediata (servicios simples). El defecto del sistema SHALL ser: aprobación manual para salones, inmediata para servicios simples.
...
El sistema NO SHALL soportar precios dinámicos (surge pricing, precios por temporada) en MVP. Los precios SHALL ser fijos según configuración del proveedor.

#### Scenario: Proveedor intenta configurar precio dinámico

- GIVEN que un proveedor busca opciones de precio dinámico
- WHEN accede a configuración de precios
- THEN NO SHALL ver opciones de surge pricing o ajuste por temporada
- AND SHALL ver mensaje de "Próximamente" o similar

### Requirement: Concurrencia — Campo Numérico Simple

La concurrencia SHALL ser un campo numérico simple en el alta del servicio. El sistema NO SHALL implementar calendario de inventario fino ni lógica de disponibilidad por slot.

#### Scenario: Configuración de concurrencia

- GIVEN que un proveedor configura concurrencia
- WHEN ingresa un número (ej: 3)
- THEN el sistema SHALL limitar reservas simultáneas a ese número
- AND NO SHALL haber gestión de inventario por slot específico
...
### Requirement: Permiso de Alcohol — No Bloqueante

El permiso de alcohol NO SHALL bloquear la reserva. Si no se confirma H-5 antes del evento, el sistema SHALL notificar y preguntar continuar o cancelar. Cancelar aplica política del proveedor.

#### Scenario: Decisión del usuario a H-5

- GIVEN que el permiso no está confirmado a H-5
- WHEN el usuario recibe notificación
- THEN SHALL poder elegir "Continuar" o "Cancelar"
