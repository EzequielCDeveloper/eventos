...
- [ ] 1.5 Create `backend/src/middleware/auth.ts` — JWT verification via jsonwebtoken, extract user.id/role/segment into req.user, reject expired/invalid tokens with 401, reject soft-deleted users (deleted_at set) (BR-002.1–BR-002.3, BR-002.6)
- [ ] 1.6 Create `backend/src/middleware/requireRole.ts` — Role-based access: requireRole('prestador'), requireRole('administrador'); returns 403 on mismatch (BR-002.3, BR-002.4)
- [ ] 1.7 Create `backend/src/middleware/errorHandler.ts` — Global error handler: map AppError to { error: { code, message, details? } }; standard HTTP status codes (BR-003.1–BR-003.3)
- [ ] 1.8 Create `backend/src/middleware/validate.ts` — Zod schema validation middleware for request body/query (BR-003.1, UR-008.5)
- [ ] 1.9 Create `backend/src/middleware/rateLimiter.ts` — express-rate-limit: 10 req/min on auth endpoints, 100 req/min on API (BR-014.3, UR-008.3)
- [ ] 1.10 Create `backend/src/routes/v1/health.routes.ts` — GET /api/v1/health returns 200 with { status: "ok", db: "connected" } (BR-014.1, UR-010.1)
- [ ] 1.11 Create `backend/src/app.ts` — Express app: helmet → cors → express.json → rateLimiter → routes → errorHandler (D-008)
- [ ] 1.12 Create `backend/src/server.ts` — HTTP server + Socket.IO attach, SIGTERM graceful shutdown (D-006, D-010, BR-014.5)

## Phase 2: Data Layer & Types (Slice S1 continued)

- [ ] 2.1 Create `backend/prisma/schema.prisma` — Generate via `prisma db pull` from database_schema.sql; all 30+ tables mapped with correct types: DECIMAL(10,2) for MXN, ENUMs for closed sets, JSON columns, soft delete via deleted_at (BR-004.1–BR-004.6, D-001)
- [ ] 2.2 Create `backend/prisma/migrations/` — Versioned SQL migration from introspected schema, up/down pairs, idempotent (BR-004.7)
- [ ] 2.3 Create `backend/prisma/seed.ts` — Seed data: default amenities, event types, initial commission rate (BR-004.8)
- [ ] 2.4 Create `backend/src/types/models.ts` — TypeScript types matching Prisma models for all 30+ entities (UR-001.5)
- [ ] 2.5 Create `backend/src/types/api.ts` — ApiResponse<T> envelope, ErrorCode union, pagination meta types (BR-001.4, BR-003.3, UR-001.1)
- [ ] 2.6 Create `backend/src/types/express.d.ts` — Extend Express Request with user: { id, role, segment } (BR-002.2)
...
- [ ] 10.1 Create `frontend/package.json` — React 18 + Vite 5 + React Router v6 + Zustand + TanStack Query + Tailwind CSS + Radix UI + Socket.IO client + Conekta.js + axios (D-009)
- [ ] 10.2 Create `frontend/vite.config.ts` — Vite config with React plugin, proxy /api to backend in dev, Tailwind CSS plugin (D-009)
- [ ] 10.3 Create `frontend/tailwind.config.js` — Tailwind config with custom theme, responsive breakpoints, Radix UI integration (FR-015.1, FR-015.3)
- [ ] 10.4 Create `frontend/tsconfig.json` — TypeScript strict mode, path aliases (@/ → src/) (D-009)
- [ ] 10.5 Create `frontend/src/types/models.ts` — Mirror backend types for all 30+ entities (UR-001.5)
- [ ] 10.6 Create `frontend/src/types/api.ts` — ApiResponse<T>, ErrorCode, pagination types matching backend (UR-001.1, BR-003.3)

## Phase 11: Frontend Platform & Shared (Slice S6 continued)

- [ ] 11.1 Create `frontend/src/lib/api.ts` — Axios instance: JWT interceptor (attach Authorization: Bearer), 401→redirect /login, retry on 5xx with exponential backoff, error handling interceptor (FR-014.1–FR-014.4)
- [ ] 11.2 Create `frontend/src/lib/socket.ts` — Socket.IO client: auth: { token }, auto-reconnect, room management per conversation, event listeners (FR-009.2, D-006)
- [ ] 11.3 Create `frontend/src/stores/authStore.ts` — Zustand store: JWT, user profile, role; persist to localStorage; actions: login, logout, setUser (FR-013.1–FR-013.3)
- [ ] 11.4 Create `frontend/src/stores/uiStore.ts` — Zustand store: theme, sidebar state, active modals (FR-013.1)
- [ ] 11.5 Create `frontend/src/components/layout/AppLayout.tsx` — Client layout: 5-tab bottom nav (Inicio, Favoritos, Rentas, Chat, Perfil); secondary nav on Inicio (Salones/Sonidos/Servicios) (FR-001.1–FR-001.4)
- [ ] 11.6 Create `frontend/src/components/layout/ProviderLayout.tsx` — Provider layout: 5-tab dashboard (Hoy, Mensajes, Calendario, Anuncios, Estadísticas) (FR-002.1–FR-002.2)
- [ ] 11.7 Create `frontend/src/components/layout/AdminLayout.tsx` — Admin layout: 5 function areas (FR-003.1–FR-003.2)
- [ ] 11.8 Create `frontend/src/components/ui/` — Shared primitives: Button, Dialog, Dropdown, Toast, Input, Select, Badge, Card (Radix UI wrappers) (FR-015.2)
- [ ] 11.9 Create `frontend/src/components/routing/ProtectedRoute.tsx` — Route guard: check authStore.JWT, redirect unauthenticated to /login; role-based: client/provider/admin render correct layout (FR-017.1–FR-017.3)
- [ ] 11.10 Create `frontend/src/App.tsx` — React Router v6 nested routes: / → client routes, /provider/* → provider routes, /admin/* → admin routes; lazy loading per feature (FR-017.1, FR-017.4)

## Phase 12: Frontend Client Features (Slice S6 continued)

- [ ] 12.1 Create `frontend/src/features/search/SearchPage.tsx` — Multi-filter search: date, capacity, zone, budget, event type, pool, internet, rating; filter persistence across navigation; results: photo, title, rating, price, capacity, location (FR-004.1–FR-004.3)
- [ ] 12.2 Create `frontend/src/features/search/hooks.ts` — useSearch: React Query hook for search API; useFilters: Zustand store for filter state (FR-013.4, FR-004.2)
- [ ] 12.3 Create `frontend/src/features/booking/ServiceDetailPage.tsx` — Gallery (min 5 photos, swipeable), pricing, rating, amenities, extras, time slots, cancellation policy, reviews, favorite toggle (FR-005.1–FR-005.6)
- [ ] 12.4 Create `frontend/src/features/booking/BookingFlow.tsx` — 6-step flow: date/time → extras → price summary (client-visible: rent+taxes, commission hidden) → Conekta.js payment → contract (salon) → confirmation; price summary shows cancellation policy; alcohol permit prompt at H-5 (FR-006.1–FR-006.9, FR-007.1–FR-007.4)
- [ ] 12.5 Create `frontend/src/features/profile/ProfilePage.tsx` — User profile, settings, ARCO rights form, privacy consent, T&C acceptance, cookie consent (FR-016.1–FR-016.6)
- [ ] 12.6 Create `frontend/src/features/chat/ChatPage.tsx` — Conversation list + detail; real-time messages via Socket.IO; voice note recording/playback (max 120s); read receipts (FR-009.1–FR-009.6)
- [ ] 12.7 Create `frontend/src/features/notifications/NotificationCenter.tsx` — In-app notification center: read/unread state, critical highlights, badge count, push handling (FR-008.1–FR-008.5)
- [ ] 12.8 Create `frontend/src/features/favorites/FavoritesPage.tsx` — Favorites list: persistent across sessions, synced with backend (FR-012.1)
- [ ] 12.9 Create `frontend/src/features/rentals/RentalHistoryPage.tsx` — Tabs: Active, In-progress, Completed, Cancelled; review form: conditional on payment complete AND event_date < now (FR-012.2–FR-012.3)
