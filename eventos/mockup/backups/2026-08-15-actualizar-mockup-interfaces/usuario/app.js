...
(function () {
  ...
  document.addEventListener('click', function (event) {
    const target = event.target.closest('[data-action], [data-nav], [data-open-detail], [data-slot], [data-category], [data-chip-close], [data-fav], [data-tab]');
    if (!target) return;
  ...
  /* ---------- Perfil desde la sesión activa (C3) ---------- */
  // Re-render del perfil: nombre/email del usuario logueado + coherencia
  // badge/estado (invariante "nunca ambos estados distintos"). Se ejecuta en
  // el init (carga inicial) y en cada fe:session-changed (login/registro
  // desde el gate sin recargar la página).
  function renderPerfil(user) {
    const u = user || window.FEStore.getCurrentUser();
    if (!u) return;

    const nombreEl = $('perfil-nombre');
  ...
  // Re-render tras login/registro desde el gate: auth.js dispara
  // fe:session-changed en cuanto oculta el overlay (con la sesión persistida).
